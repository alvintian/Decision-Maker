...
.dropTableIfExists('users')
	...
	function createDummyData() {
		return Promise.all([knex('users').insert([
			{email: "DooDoo@CuCu.ca", name: "DooDoo",test:""},
			{email: "BooBoo@gmail.com", name: "BooBoo"}]),
			knex('poll').insert([
			{admin_url: "admin/xyz", poll_url: "/xyz",
			poll_question: "do you like PooPoo?", user_id: 1}]),
			knex('option').insert([
			{choice_description: "Yes", score: "0", poll_id: 1},
			{choice_description: "No", score: "1", poll_id: 1},
			{choice_description: "Maybe", score: "2", poll_id: 1}])
			])
};
};
